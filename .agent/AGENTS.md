...
- **Giải pháp:**
  ...
  - **Cập nhật giao diện Frontend**:
    - Đăng ký module `'kds'` trong `staffs.component.ts`.
...
- **Giải pháp:**
  ...
  - **Cập nhật giao diện động ở Frontend:** 
...
- **Giải pháp:**
  - **Backend API & Database:**
    ...
    - Tạo Model `TaxPeriodDetail` tự động sinh UUID v7 khi tạo mới.
    - Đăng ký routes `GET /tax/periods/{id}/details` và `POST /tax/periods/{id}/details` trong `api.php`.
...
- **Giải pháp:**
  - Tạo mới component [CustomRadioComponent](file:///d:/git/cafe-blockchain/cafe-blockchain-web/src/app/shared/components/custom-radio/custom-radio.component.ts) độc lập (standalone):
  ...
  - Cập nhật [create-tax-period-modal.component.html](file:///d:/git/cafe-blockchain/cafe-blockchain-web/src/app/features/tax/components/create-tax-period-modal/create-tax-period-modal.component.html):
    ...
    - Thay thế bằng `<app-custom-radio>` kết hợp với binding `ngModel`.
  - Cập nhật [create-tax-period-modal.component.ts](file:///d:/git/cafe-blockchain/cafe-blockchain-web/src/app/features/tax/components/create-tax-period-modal/create-tax-period-modal.component.ts):
    - Đăng ký `CustomRadioComponent` trong mảng `imports`.
...
- **Giải pháp:**
  ...
  - **Tối ưu hóa Frontend (`tax.component.ts`):** 
    ...
    - Loại bỏ việc gọi `loadInitialData()` tự động trong `ngOnInit()`, chỉ giữ lại việc cập nhật tab từ route.
...
- **Giải pháp:**
  ...
  - **Sửa lỗi biên dịch Frontend & Hiển thị Giao diện:**
    - Cập nhật `settingsSubTab` signal trong `settings.component.ts` để bổ sung kiểu `'tax'`.
...
- **Giải pháp:**
  - **Tái cấu trúc Backend:** Di chuyển và chuẩn hóa logic xử lý URL hình ảnh thành hàm static `normalizeImageUrl($imageUrl)` trong Model [Product.php](file:///d:/git/cafe-blockchain/cafe-blockchain-api/app/Domain/Entities/Product.php).
...
- **Giải pháp:**
  ...
  - **Đăng ký route:** Thêm route `POST /products/upload-image` trong [api.php](file:///d:/git/cafe-blockchain/cafe-blockchain-api/routes/api.php).
...
- **Giải pháp:**
  ...
  - Chạy `php -l` kiểm tra cú pháp PHP thành công 100%.
...
- **Giải pháp:**
  ...
  - Chạy `php -l` kiểm tra cú pháp backend thành công 100%.
...
- **Giải pháp:**
  ...
  - Chạy `php -l` kiểm tra cú pháp thành công 100%.
  - Chạy `php artisan cache:clear` dọn dẹp bộ nhớ đệm hệ thống thành công.


### Yêu cầu: Kiểm tra và chuẩn hóa các nút bấm native trong SaaS Admin sang app-button
...
- **Giải pháp:**
  ...
  - Chạy thành công `php artisan cache:clear` và kiểm tra cú pháp PHP.
...
### Yêu cầu: Triển khai Backend Cache (BE Cache) cho Cửa hàng
...
- **Giải pháp:**
  ...
  - **Frontend API Service**: Cập nhật [api.service.ts](file:///d:/git/cafe-blockchain/cafe-blockchain-web/src/app/core/services/api.service.ts), bổ sung phương thức `adminMaintenanceClearAll()`.
...
- **Giải pháp:**
  ...
  - Chạy `php -l` kiểm tra cú pháp thành công 100% cho các controller.
...
- **Giải pháp:**
  - Cập nhật backend [api.php](file:///d:/git/cafe-blockchain/cafe-blockchain-api/routes/api.php): Di chuyển `GET /admin/staffs` và `GET /admin/payment-methods` từ nhóm `EnsureIsSuperAdmin` sang nhóm `EnsureIsSystemAdmin`. Các route write (POST/PUT/DELETE) giữ nguyên trong nhóm Super Admin.
...
- **Giải pháp:**
  - Cập nhật [admin-saas.component.ts](file:///d:/git/cafe-blockchain/cafe-blockchain-web/src/app/features/admin/admin-saas.component.ts): Reset signals tìm kiếm và phân trang của nhân viên khi chuyển tab.
  - Cập nhật [admin-saas.component.html](file:///d:/git/cafe-blockchain/cafe-blockchain-web/src/app/features/admin/admin-saas.component.html):
...
- **Giải pháp:**
  ...
  - Cập nhật API routes và controllers tại backend: 
    ...
    - Mở rộng api trả về `admin_role` ('super_admin', 'staff' hoặc null).
...
- **Giải pháp:**
  ...
  - Cập nhật API routes và controllers tại backend: Đăng ký API Super Admin CRUD và API public cho chủ quán.
  - Cập nhật frontend:
...
- **Giải pháp:**
  ...
  - **Đồng bộ giao diện modal chi tiết:** Cập nhật [subscription-request-detail-modal.component.html](file:///d:/git/cafe-blockchain/cafe-blockchain-web/src/app/features/admin/components/subscription-request-detail-modal/subscription-request-detail-modal.component.html):
...
- **Giải pháp:**
  ...
  - Cập nhật [store.component.html](file:///d:/git/cafe-blockchain/cafe-blockchain-web/src/app/features/storefront/pages/store/store.component.html) thay thế khối code drawer cũ bằng thẻ `<app-store-cart-drawer>`.
  - Chạy `npm run build` thành công 100%.
...
- **Giải pháp:**
  - Cập nhật [Cors.php](file:///d:/git/cafe-blockchain/cafe-blockchain-api/app/Http/Middleware/Cors.php): Triển khai dynamic whitelist từ `.env` (`ALLOWED_ORIGINS`).
...
### Yêu cầu: Chuyển Cloudflare Turnstile toggle sang Database + Super Admin UI
...
- **Giải pháp:**
  ...
  - Đăng ký interceptor trong [app.config.ts](file:///d:/git/cafe-blockchain/cafe-blockchain-web/src/app/app.config.ts).
...
- **Giải pháp:**
  ...
  - **Chiều đồng bộ dữ liệu rõ ràng:**
    - `staffs → users` (một lần duy nhất khi đăng nhập đầu, nếu users trống)
...
- **Giải pháp:**
  ...
  - Cập nhật [store.component.ts](file:///d:/git/cafe-blockchain/cafe-blockchain-web/src/app/features/storefront/pages/store/store.component.ts) và [store.component.html](file:///d:/git/cafe-blockchain/cafe-blockchain-web/src/app/features/storefront/pages/store/store.component.html):
    ...
    - Tự động cuộn container để hiển thị danh mục active nếu bị che khuất.
...
- **Giải pháp:**
  ...
  - Sửa đổi [financial-detail-modal.component.html](file:///d:/git/cafe-blockchain/cafe-blockchain-web/src/app/features/financials/components/financial-detail-modal/financial-detail-modal.component.html):
...
### Yêu cầu: Tiếp tục hoàn thành dọn dẹp các modal tĩnh còn lại và kiểm thử compile toàn cục
...
- **Giải pháp:**
  ...
  - Cập nhật [admin-saas.component.html](file:///d:/git/cafe-blockchain/cafe-blockchain-web/src/app/features/admin/admin-saas.component.html): Thay thế khối toggle switch cũ bằng component `<app-custom-switch [checked]="turnstileEnabled()" [disabled]="!turnstileHasKey() && !turnstileEnabled()" (checkedChange)="toggleTurnstile($event)" type="compact"></app-custom-switch>`.
  - Chạy `npm run build` thành công 100%.
...
- **Giải pháp:**
  - Tạo mới component nội bộ [FloatingCartBarComponent](file:///d:/git/cafe-blockchain/cafe-blockchain-web/src/app/features/storefront/components/floating-cart-bar) gồm:
  ...
  - Cập nhật [store.component.html](file:///d:/git/cafe-blockchain/cafe-blockchain-web/src/app/features/storefront/pages/store/store.component.html): Thay thế khối div cũ bằng thẻ `<app-floating-cart-bar [cartCount]="cartCount()" [cartTotal]="cartTotal()" (checkout)="isCartOpen.set(true)"></app-floating-cart-bar>`.
  - Chạy `npm run build` thành công 100%.
...
- **Giải pháp:**
  ...
  - **Đồng bộ Case-sensitivity ví khách hàng:**
    - Cập nhật [CustomerController.php](file:///d:/git/cafe-blockchain/cafe-blockchain-api/app/Http/Controllers/CustomerController.php) chuyển `wallet_address` thành chữ thường (`strtolower`) khi lưu trữ.
  ...
  - **Tối ưu UI Host Display cho Custom Components:**
...
- **Giải pháp:**
  ...
  - **Nghiệm thu:**
    - Chạy kiểm tra cú pháp Laravel (`php artisan route:list`) thành công 100%.
