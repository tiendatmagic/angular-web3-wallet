...
- **Phân tích kỹ thuật & Nguyên nhân gốc rễ:**
  1. **Hiện tượng Boundary Hover Chatter (Vòng lặp mất/nhận hover vô tận):**
     ...
     - Khi con trỏ chuột chạm vào mép dưới cùng (bottom boundary) của avatar, sự kiện `:hover` kích hoạt.
     - Avatar dịch chuyển lên trên 6-8px, kéo theo mép dưới đáy của avatar nhấc bổng lên cao hơn vị trí con trỏ chuột.
     - Con trỏ chuột rơi vào khoảng trống bên dưới vừa tạo ra -> trình duyệt hủy bỏ `:hover`.
     - Avatar rơi ngược trở lại vị trí gốc (0px) -> mép dưới lại chạm trúng con trỏ chuột -> lại kích hoạt `:hover` -> lại nhấc lên -> lại mất chuột -> lại rơi xuống...
...
- **Phân tích nguyên nhân gốc rễ:**
  1. **Lỗi lưu toạ độ tĩnh (`DOMRect`):** Khi hover mở Submenu, hệ thống lưu `this.lastSubmenuTriggerRect = getBoundingClientRect()`. Khi người dùng cuộn trang (`window:scroll`), Menu chính được cập nhật lại toạ độ theo nút trigger mới, nhưng Submenu lại vẫn đọc toạ độ từ `lastSubmenuTriggerRect` cũ (giá trị pixel cố định trước khi cuộn), khiến Submenu bị ghim chết ở vị trí cũ dưới đáy màn hình.
- **Giải pháp triệt để:**
  1. **Lưu tham chiếu trực tiếp phần tử cha (`activeSubmenuTriggerEl: HTMLElement`):**
     - Thay vì lưu `DOMRect` tĩnh, hệ thống lưu trực tiếp `HTMLElement` của item cha (`event.currentTarget`).
     - Hàm `updateSubmenuPosition()` luôn gọi `this.activeSubmenuTriggerEl.getBoundingClientRect()` theo thời gian thực (real-time live measurement).
     - Khi cuộn trang (`window:scroll`) hoặc cập nhật view (`ngAfterViewChecked`, `popoverEl.scroll`), Submenu và Menu chính luôn bám sát nhau 1:1 theo từng pixel.
  2. **Tự động đóng Submenu khi cha ra ngoài Viewport:** Nếu item cha bị cuộn biến mất khỏi màn hình (`triggerRect.bottom <= 0` hoặc `triggerRect.top >= innerHeight`), Submenu sẽ tự động đóng ngay lập tức để tránh hiển thị mồ côi.
  3. **Vị trí Anchor chuẩn:** Submenu bám sát mép phải item cha `triggerRect.right + 4` (hoặc lật trái nếu chạm mép phải) và bám thẳng hàng đỉnh item cha (lật lên bám chân nếu tràn đáy màn hình).
...
- **Giải pháp:**
  ...
  11. **Dọn dẹp các component dùng chung khác:** Loại bỏ comment chú thích đơn giản / ControlValueAccessor rác trong [accordion.component.ts](file:///d:/git/angular-web3-wallet/src/app/shared/components/accordion/accordion.component.ts), [custom-checkbox.component.ts](file:///d:/git/angular-web3-wallet/src/app/shared/components/custom-checkbox/custom-checkbox.component.ts), [custom-slider.component.ts](file:///d:/git/angular-web3-wallet/src/app/shared/components/custom-slider/custom-slider.component.ts), [custom-input.component.ts](file:///d:/git/angular-web3-wallet/src/app/shared/components/custom-input/custom-input.component.ts), [custom-radio.component.ts](file:///d:/git/angular-web3-wallet/src/app/shared/components/custom-radio/custom-radio.component.ts), và [custom-search-input.component.ts](file:///d:/git/angular-web3-wallet/src/app/shared/components/custom-search-input/custom-search-input.component.ts).
  12. **Dọn dẹp HTML:** Loại bỏ toàn bộ comment cấu trúc rườm rà `<!-- ... -->` và các separator trong [sidebar.component.html](file:///d:/git/angular-web3-wallet/src/app/shared/layout/sidebar/sidebar.component.html), [header.component.html](file:///d:/git/angular-web3-wallet/src/app/shared/layout/header/header.component.html), [tx-speed-selector.component.html](file:///d:/git/angular-web3-wallet/src/app/shared/components/tx-speed-selector/tx-speed-selector.component.html), [toast.component.html](file:///d:/git/angular-web3-wallet/src/app/shared/components/toast/toast.component.html), [tab-group.component.html](file:///d:/git/angular-web3-wallet/src/app/shared/components/tab-group/tab-group.component.html), [pagination.component.html](file:///d:/git/angular-web3-wallet/src/app/shared/components/pagination/pagination.component.html), [modal-wrapper.component.html](file:///d:/git/angular-web3-wallet/src/app/shared/components/modal/modal-wrapper.component.html), [modal.component.html](file:///d:/git/angular-web3-wallet/src/app/shared/components/modal/modal.component.html), [page-header.component.html](file:///d:/git/angular-web3-wallet/src/app/shared/components/page-header/page-header.component.html) và [skeleton-loader.component.html](file:///d:/git/angular-web3-wallet/src/app/shared/components/skeleton-loader/skeleton-loader.component.html).
  13. **Kiểm tra:** `npm run build` thành công, không lỗi biên dịch.
...
- **Giải pháp:**
  1. **Tạo Directive standalone [ripple.directive.ts](file:///d:/git/angular-web3-wallet/src/app/shared/components/ripple/ripple.directive.ts):**
     * Lắng nghe sự kiện `mousedown` và `touchstart`, có cơ chế chặn kích hoạt đúp trên thiết bị di động.
     ...
     * Thiết lập `overflow: visible` cho container cha khi `unbounded` là true để đảm bảo không bị cắt cụt bởi class CSS, ngược lại tự động thêm `overflow: hidden`.
     * Chạy logic dọn dẹp phần tử ngoài Angular Zone bằng `setTimeout` sau 500ms.
  2. **Thêm style cho ripple** tại [styles.scss](file:///d:/git/angular-web3-wallet/src/styles.scss):
     * Định nghĩa lớp `.app-ripple-element` với `position: absolute`, `border-radius: 50%`. Tăng độ mờ mặc định lên `0.3` để hiển thị rõ nét trên nền sáng.
     * Chia tách animation: `@keyframes app-ripple-scale` (phóng to nhanh bằng cubic-bezier) và `@keyframes app-ripple-fade` (mờ dần đều đặn bằng linear) chạy song song trong 500ms giúp tăng độ tương phản rõ rệt trên nút Primary Button.
  3. **Tích hợp Showcase vào trang chủ**:
     * Đăng ký `RippleDirective` trong `imports` của [home.component.ts](file:///d:/git/angular-web3-wallet/src/app/features/home/home.component.ts) và tạo 4 signals điều khiển cấu hình.
     * Cập nhật [home.component.html](file:///d:/git/angular-web3-wallet/src/app/features/home/home.component.html): Thêm **Card 13: Custom Ripple Directive** chứa hộp tương tác dùng thử hiệu ứng sóng nước, 3 switch tùy biến thuộc tính, 4 nút chọn nhanh màu (đã sửa lỗi class viền ring tương ứng từng màu và sửa lỗi opacity màu tùy chỉnh) và các button mẫu áp dụng directive trực quan (đã sửa lỗi class viền ring tương ứng từng màu và sửa lỗi opacity màu tùy chỉnh, đồng thời đồng bộ switch unbounded/centered cho Icon Button tròn để tránh lỗi viền và đổi các mã màu tùy chỉnh dạng rgba tĩnh thành màu đặc hex để không bị triệt tiêu độ mờ).
...
### Yêu cầu: Xây dựng và tích hợp component Aura tạo hiệu ứng viền phát sáng xoay chuyển động (Aura component)
- **Nội dung yêu cầu:** Tạo component mới đặt tên là `aura` dựa trên mẫu DaisyUI Aura component, sử dụng màu chủ đạo (Primary Accent) và phụ trợ (Secondary Accent) làm viền phát sáng xung quanh. Tích hợp showcase demo lên trang chủ.
- **Giải pháp:**
  1. **Tạo component standalone [aura.component.ts](file:///d:/git/angular-web3-wallet/src/app/shared/components/aura/aura.component.ts):**
     * Định nghĩa các biến cấu hình đầu vào `variant`, `size`, `glow`, `speed`, `radius`, `paused`.
     * Sử dụng hai lớp nền phụ tuyệt đối: `.aura-glow` (tỏa sáng mờ, `filter: blur()`) và `.aura-border` (viền sắc nét, `inset: -1.5px`) xoay chuyển động tròn.
     * Sử dụng `@property --aura-angle` để trình duyệt hỗ trợ nội suy góc xoay làm mượt chuyển động xoay gradient màu (`conic-gradient`) mà không cần thay đổi cấu trúc DOM vật lý.
     * Tận dụng các biến màu CSS có sẵn `var(--color-primary)` và `var(--color-secondary)` cho variant `dual` (mặc định) để thể hiện đúng màu chủ đạo và phụ trợ của ứng dụng.
  2. **Cập nhật [home.component.ts](file:///d:/git/angular-web3-wallet/src/app/features/home/home.component.ts):**
     * Import `AuraComponent` và đăng ký trong imports của component trang chủ.
     * Thêm các signal điều khiển cấu hình trực tiếp: `demoAuraVariant`, `demoAuraSize`, `demoAuraGlow`, `demoAuraPaused`, `demoAuraSpeed`, `demoAuraRadius`.
  3. **Cập nhật [home.component.html](file:///d:/git/angular-web3-wallet/src/app/features/home/home.component.html):**
     * Bổ sung **Card 14: Custom Aura Component (DaisyUI 5.6+)** vào cuối danh sách showcase demo của trang chủ.
     * Thiết kế bảng điều khiển tương tác trực tiếp (chọn màu sắc, chọn size từ xs đến xl, switch tắt bật glow/pause, chọn tốc độ, chọn bo góc) và hiển thị các ví dụ áp dụng thực tế trên nút bấm, avatar tròn, thẻ VIP.
- **Kiểm tra:**
